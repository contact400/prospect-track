...
window.opsGenerateOfferPDF = function(lid, oid) {
  ...
  document.getElementById("opsModalContent").innerHTML = `
    ...
    <div class="mbody-ops" style="display:flex;gap:12px;padding:1.5rem;">
      <button class="ops-lang-btn" onclick="closeAllModals();opsGenerateOfferPDFLang('${lid}','${oid}','fr')">🇫🇷 Français</button>
      <button class="ops-lang-btn" onclick="closeAllModals();opsGenerateOfferPDFLang('${lid}','${oid}','en')">🇬🇧 English</button>
    </div>`;
  openModal("opsModal");
};

window.opsGenerateOfferPDFLang = function(lid, oid, lang) {
  const l = opsListings.find(x=>x.id===lid);
  const o = (l?.offers||[]).find(x=>x.id===oid);
  if (!l||!o) return;
  const T = lang==="en" ? {
    title:"Purchase Offer Summary", generated:"Generated on", property:"Property",
    listingPrice:"Listing price", offer:"Offer", buyer:"Buyer", agent:"Representing agent",
    offeredPrice:"Offered price", validity:"Offer validity", status:"Status",
    acceptedOn:"Accepted on", conditions:"Conditions", inspection:"Inspection",
    financing:"Financing", docReview:"Document review", other:"Other condition",
    docs:"Documents required", deadline:"deadline from acceptance", days:"days",
    dates:"Dates & Occupancy", notary:"Desired notary date", occupancy:"Desired occupancy date",
    rent:"Rent if delay between notary/occupancy", inclExcl:"Inclusions & Exclusions",
    inclusions:"Inclusions", exclusions:"Exclusions", footer:"BACHA Real Estate Group · Generated by Track",
    statuses:{pending:"Pending",accepted:"Accepted",second:"2nd rank",refused:"Refused"}
  } : {
    title:"Résumé d'offre d'achat", generated:"Généré le", property:"Propriété",
    listingPrice:"Prix inscrit", offer:"Offre", buyer:"Acheteur", agent:"Courtier représentant",
    offeredPrice:"Prix offert", validity:"Validité de l'offre", status:"Statut",
    acceptedOn:"Date d'acceptation", conditions:"Conditions", inspection:"Inspection",
    financing:"Financement", docReview:"Revue de documents", other:"Autre condition",
    docs:"Documents requis", deadline:"à compter de l'acceptation", days:"jours",
    dates:"Dates & Occupation", notary:"Date du notaire souhaitée", occupancy:"Date d'occupation souhaitée",
    rent:"Loyer si délai notaire/occupation", inclExcl:"Inclusions & Exclusions",
    inclusions:"Inclusions", exclusions:"Exclusions", footer:"BACHA Groupe Immobilier · Document généré par Track",
    statuses:{pending:"En attente",accepted:"Acceptée",second:"2e rang",refused:"Refusée"}
  };
  const locale = lang==="en" ? "en-CA" : "fr-CA";
  const fmtDate = ds => { if(!ds) return "—"; const [y,m,d]=ds.slice(0,10).split("-").map(Number); const dt=new Date(y,m-1,d); return dt.toLocaleDateString(locale,{day:"numeric",month:"long",year:"numeric"}); };
  const fmtDeadline = (ds,days) => { if(!ds&&!days) return "—"; if(ds) return fmtDate(ds)+(days?` (${days} ${T.days})`:""); return `${days} ${T.days} ${T.deadline}`; };
  const html = `<!DOCTYPE html><html lang="${lang}"><head><meta charset="UTF-8">
  <title>${T.title} — ${l.addr}</title>
  <style>
    ...
    <div class="ops-conds-card">
      <div class="ops-conds-hd">
        ...
        <div style="display:flex;gap:6px;">
          ...
          <button class="btn-primary" style="font-size:12px;padding:4px 12px;" onclick="opsActivityReportPicker('${l.id}')">Rapport PDF ↗</button>
        </div>
      </div>
...
window.opsActivityReportPicker = function(lid) {
  document.getElementById("opsModalContent").innerHTML = `
    <div class="modal-header"><div class="modal-title">Langue du rapport</div><button class="close-x" onclick="closeAllModals()">×</button></div>
    <div class="mbody-ops" style="display:flex;gap:12px;padding:1.5rem;">
      <button class="ops-lang-btn" onclick="closeAllModals();opsGenerateActivityReport('${lid}','fr')">🇫🇷 Français</button>
      <button class="ops-lang-btn" onclick="closeAllModals();opsGenerateActivityReport('${lid}','en')">🇬🇧 English</button>
    </div>`;
  openModal("opsModal");
};

window.opsGenerateActivityReport = function(lid, lang) {
  const l = opsListings.find(x=>x.id===lid);
  if (!l) return;
  const acts = opsGetActivity(lid);
  const locale = lang==="en" ? "en-CA" : "fr-CA";
  const fmtDate = ds => { if(!ds) return "—"; const [y,m,d]=ds.slice(0,10).split("-").map(Number); const dt=new Date(y,m-1,d); return dt.toLocaleDateString(locale,{day:"numeric",month:"long",year:"numeric"}); };
  const fmtDeadline = ds => { if(!ds) return "—"; const [y,m,d]=ds.slice(0,10).split("-").map(Number); const dt=new Date(y,m-1,d); return dt.toLocaleDateString(locale,{day:"numeric",month:"long",year:"numeric"}); };

  const T = lang==="en" ? {
    reportTitle:"Listing Activity Report", generatedOn:"Generated on",
    property:"Property", listingPrice:"Listing price", offerPrice:"Accepted offer price",
    agent:"Agent", status:"Status",
    activitySummary:"Activity Summary", totalVisits:"Total visits",
    openHouses:"Open houses", interested:"Interested", secondShowings:"2nd showings",
    followUpsPending:"Follow-ups pending", avgRating:"Average rating",
    sources:"Visit Sources", offerIntention:"Offer Intention",
    direct:"Direct", broker:"Broker", immocontact:"Immocontact",
    realtorca:"Realtor.ca", openhouse:"Open house", other:"Other",
    yes:"Yes", maybe:"Maybe", no:"No",
    offersReceived:"Offers Received", noOffers:"No offers recorded.",
    buyer:"Buyer", representing:"Representing agent", price:"Price",
    validity:"Validity", accepted:"Accepted", pending:"Pending",
    secondRank:"2nd rank", refused:"Refused",
    conditionsTitle:"Offer Conditions", inspection:"Inspection",
    financing:"Financing", docReview:"Document review", otherCond:"Other",
    deadline:"Deadline", fulfilled:"Fulfilled", pending2:"Pending",
    visitLog:"Visit Log", noVisits:"No activity recorded.",
    visitor:"Visitor", source:"Source", ratings:"Ratings",
    intention:"Intention", followup:"Follow-up", notes:"Notes",
    overall:"Overall", interior:"Interior", exterior:"Exterior", priceR:"Price",
    footer:"BACHA Real Estate Group · Generated by Track"
  } : {
    reportTitle:"Rapport d'activité — Inscription", generatedOn:"Généré le",
    property:"Propriété", listingPrice:"Prix inscrit", offerPrice:"Prix offre acceptée",
    agent:"Courtier", status:"Statut",
    activitySummary:"Résumé d'activité", totalVisits:"Visites totales",
    openHouses:"Visites libres", interested:"Intéressés", secondShowings:"2es visites",
    followUpsPending:"Suivis requis", avgRating:"Note moyenne",
    sources:"Sources de visite", offerIntention:"Intention d'offre",
    direct:"Direct", broker:"Courtier", immocontact:"Immocontact",
    realtorca:"Realtor.ca", openhouse:"Visite libre", other:"Autre",
    yes:"Oui", maybe:"Peut-être", no:"Non",
    offersReceived:"Offres reçues", noOffers:"Aucune offre enregistrée.",
    buyer:"Acheteur", representing:"Courtier représentant", price:"Prix",
    validity:"Validité", accepted:"Acceptée", pending:"En attente",
    secondRank:"2e rang", refused:"Refusée",
    conditionsTitle:"Conditions de l'offre", inspection:"Inspection",
    financing:"Financement", docReview:"Revue de documents", otherCond:"Autre condition",
    deadline:"Date limite", fulfilled:"Levée", pending2:"En cours",
    visitLog:"Journal de visites", noVisits:"Aucune activité enregistrée.",
    visitor:"Visiteur", source:"Source", ratings:"Évaluations",
    intention:"Intention", followup:"Suivi", notes:"Notes",
    overall:"Globale", interior:"Intérieur", exterior:"Extérieur", priceR:"Prix",
    footer:"BACHA Groupe Immobilier · Document généré par Track"
  };

  const visits = acts.filter(a=>a.type==="visit");
  const openHouses = acts.filter(a=>a.type==="openhouse");
  const interested = acts.filter(a=>["yes","maybe"].includes(a.offerIntention)).length;
  const secondShowings = acts.filter(a=>a.secondShowing).length;
  const followUpsPending = acts.filter(a=>a.followUpRequired&&!a.followUpDone).length;
  const avgR = k => { const r=acts.filter(a=>a.ratings?.[k]); return r.length?(r.reduce((s,a)=>s+Number(a.ratings[k]),0)/r.length).toFixed(1):"—"; };

  const sourceMap = {direct:T.direct,agent:T.broker,immocontact:T.immocontact,realtorca:T.realtorca,openhouse:T.openhouse,other:T.other};
  const sourceCounts = {};
  acts.forEach(a=>{ const s=a.source||"other"; sourceCounts[s]=(sourceCounts[s]||0)+1; });
  const sourceRows = Object.entries(sourceCounts).sort((a,b)=>b[1]-a[1])
    .map(([s,n])=>`<tr><td>${sourceMap[s]||s}</td><td>${n}</td></tr>`).join("");

  const intentionCounts = {yes:0,maybe:0,no:0};
  acts.filter(a=>a.offerIntention).forEach(a=>{ if(intentionCounts[a.offerIntention]!==undefined) intentionCounts[a.offerIntention]++; });
  const intentionRows = [["yes",T.yes],["maybe",T.maybe],["no",T.no]]
    .filter(([k])=>intentionCounts[k]>0)
    .map(([k,label])=>`<tr><td>${label}</td><td>${intentionCounts[k]}</td></tr>`).join("");

  const offers = l.offers||[];
  const statusMap = {pending:T.pending,accepted:T.accepted,second:T.secondRank,refused:T.refused};
  const offersHtml = offers.length ? offers.map(o=>`
    <tr style="background:#f9f9f9;">
      <td colspan="2" style="font-weight:600;padding-top:10px;">${o.buyer||"—"} ${o.agent?`(${o.agent})`:""}</td>
    </tr>
    <tr><td>${T.price}</td><td>${o.price||"—"}</td></tr>
    <tr><td>${T.status}</td><td>${statusMap[o.status]||"—"}</td></tr>
    <tr><td>${T.validity}</td><td>${o.validity||"—"}</td></tr>
    ${o.conditions?.inspection?`<tr><td>${T.inspection}</td><td>${o.conditions.inspection} ${lang==="en"?"days":"jours"}${o.deadlines?.inspection?" — "+fmtDeadline(o.deadlines.inspection):""}</td></tr>`:""}
    ${o.conditions?.financing?`<tr><td>${T.financing}</td><td>${o.conditions.financing} ${lang==="en"?"days":"jours"}${o.deadlines?.financing?" — "+fmtDeadline(o.deadlines.financing):""}</td></tr>`:""}
    ${o.conditions?.docReview?`<tr><td>${T.docReview}</td><td>${o.conditions.docReview} ${lang==="en"?"days":"jours"}${o.deadlines?.docReview?" — "+fmtDeadline(o.deadlines.docReview):""}</td></tr>`:""}
    ${o.conditions?.other?`<tr><td>${T.otherCond}</td><td>${o.conditions.other} ${lang==="en"?"days":"jours"}${o.deadlines?.other?" — "+fmtDeadline(o.deadlines.other):""}</td></tr>`:""}
  `).join("<tr><td colspan='2'><hr style='border:none;border-top:1px solid #eee;margin:6px 0;'></td></tr>")
  : `<tr><td colspan="2" style="color:#888;">${T.noOffers}</td></tr>`;

  const conditions = (l.conditions||[]);
  const condsHtml = conditions.length ? conditions.map(c=>`
    <tr>
      <td>${c.name}</td>
      <td>${fmtDeadline(c.date)}</td>
      <td style="color:${c.done?"#1D9E75":"#BA7517"};font-weight:500;">${c.done?T.fulfilled:T.pending2}</td>
    </tr>`).join("")
  : `<tr><td colspan="3" style="color:#888;">—</td></tr>`;

  const visitRows = acts.map(a=>{
    const isOH = a.type==="openhouse";
    return `<tr>
      <td>${a.date||"—"}</td>
      <td>${isOH?(lang==="en"?"Open house":"Visite libre"):(lang==="en"?"Visit":"Visite")}</td>
      <td>${a.visitorName||a.agentName||a.totalVisitors+(lang==="en"?" visitors":" visiteurs")||"—"}</td>
      <td>${sourceMap[a.source]||a.source||"—"}</td>
      <td>${isOH?`${a.interestedCount||0} ${lang==="en"?"interested":"intéressés"}`:(a.ratings?.overall?`${a.ratings.overall}/5 ${T.overall}`:"—")}</td>
      <td>${a.offerIntention?({yes:T.yes,maybe:T.maybe,no:T.no}[a.offerIntention]||""):"—"}</td>
      <td>${a.followUpRequired?(a.followUpDone?(lang==="en"?"Done":"Fait"):(lang==="en"?"Required":"Requis")):"—"}</td>
    </tr>`;
  }).join("");

  const html = `<!DOCTYPE html><html lang="${lang}"><head><meta charset="UTF-8">
  <title>${T.reportTitle} — ${l.addr}</title>
  <style>
    body{font-family:Arial,sans-serif;font-size:12px;color:#1a1a1a;margin:32px;line-height:1.6;}
    h1{font-size:18px;color:#0C2B5E;margin-bottom:2px;}
    h2{font-size:13px;color:#0C2B5E;border-bottom:2px solid #0C2B5E;padding-bottom:4px;margin:20px 0 8px;}
    .sub{font-size:12px;color:#666;margin-bottom:20px;}
    .grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;}
    .kpi{background:#f5f5f5;border-radius:6px;padding:10px 14px;border-left:3px solid #0C2B5E;}
    .kpi-label{font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#666;}
    .kpi-val{font-size:20px;font-weight:700;color:#0C2B5E;}
    table{width:100%;border-collapse:collapse;margin-bottom:12px;}
    td,th{padding:6px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;text-align:left;}
    th{background:#f5f5f5;font-weight:600;font-size:11px;text-transform:uppercase;letter-spacing:.04em;color:#555;}
  ...
  <h1>BACHA Groupe Immobilier</h1>
  <div class="sub">${T.reportTitle} · ${T.generatedOn} ${fmtDate(new Date().toISOString().slice(0,10))}</div>

  <table>
    <tr><td>${T.property}</td><td style="font-size:15px;font-weight:700;">${l.addr}</td></tr>
    ${l.price?`<tr><td>${T.listingPrice}</td><td>${l.price}</td></tr>`:""}
    ${l.offerPrice?`<tr><td>${T.offerPrice}</td><td style="color:#1D9E75;font-weight:600;">${l.offerPrice}</td></tr>`:""}
    <tr><td>${T.agent}</td><td>${l.agent||"—"}</td></tr>
  </table>

  <h2>${T.activitySummary}</h2>
  <div class="grid">
    <div class="kpi"><div class="kpi-label">${T.totalVisits}</div><div class="kpi-val">${visits.length}</div></div>
    <div class="kpi"><div class="kpi-label">${T.openHouses}</div><div class="kpi-val">${openHouses.length}</div></div>
    <div class="kpi"><div class="kpi-label">${T.interested}</div><div class="kpi-val">${interested}</div></div>
    <div class="kpi"><div class="kpi-label">${T.secondShowings}</div><div class="kpi-val">${secondShowings}</div></div>
    <div class="kpi"><div class="kpi-label">${T.followUpsPending}</div><div class="kpi-val">${followUpsPending}</div></div>
    <div class="kpi"><div class="kpi-label">${T.avgRating}</div><div class="kpi-val">${avgR("overall")}/5</div></div>
  </div>

  ${sourceRows?`<h2>${T.sources}</h2><table><tr><th>${T.source}</th><th>#</th></tr>${sourceRows}</table>`:""}
  ${intentionRows?`<h2>${T.offerIntention}</h2><table><tr><th>${T.intention}</th><th>#</th></tr>${intentionRows}</table>`:""}

  <h2>${T.offersReceived}</h2>
  <table>${offersHtml}</table>

  ${conditions.length?`<h2>${T.conditionsTitle}</h2>
  <table><tr><th>${lang==="en"?"Condition":"Condition"}</th><th>${T.deadline}</th><th>${T.status}</th></tr>${condsHtml}</table>`:""}

  <h2>${T.visitLog}</h2>
  ${acts.length?`<table>
    <tr><th>${lang==="en"?"Date":"Date"}</th><th>${lang==="en"?"Type":"Type"}</th><th>${T.visitor}</th><th>${T.source}</th><th>${T.ratings}</th><th>${T.intention}</th><th>${T.followup}</th></tr>
    ${visitRows}
  </table>`:`<p style="color:#888;">${T.noVisits}</p>`}

  <div class="footer">${T.footer} · ${new Date().toLocaleDateString(locale)}</div>
  </body></html>`;
